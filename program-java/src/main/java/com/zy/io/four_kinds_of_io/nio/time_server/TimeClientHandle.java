...
public class TimeClientHandle implements Runnable {
    ...
    public void run() {
        ...
        //多路复用器关闭后，其上的channel和pipe等资源都会被自动去注册并关闭，所以不需要重复释放资源
        if(selector != null){
            try {
                selector.close();
            } catch (IOException e) {
    ...
    private void doConnect() throws IOException {
        ...
        if(socketChannel.connect(new InetSocketAddress(host,port))){//异步连接服务端//成功，择直接注册读状态位到多路复用器中//如果连接不成功(异步链接，返回false，说明客户端已发送sync包，服务端返回ack包，物理链路还没有建立
