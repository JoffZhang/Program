...
public class HttpXmlRequestEncoder extends AbstractHttpXmlEncoder<HttpXmlRequest> {
    ...
    @Override
    protected void encode(ChannelHandlerContext channelHandlerContext, HttpXmlRequest msg, List<Object> out) throws Exception {
        //调用父类的encode0，将业务需要发送的POJO对象Order实例通过JiBx序列化为XML字符串，随后将他封装成Netty的ByteBuf
        ByteBuf body = encode0(channelHandlerContext,msg.getBody());
        FullHttpRequest request = msg.getRequest();
        //对消息头进行判断，如果业务侧自定义和定制了消息头，则适用业务侧设置的HTTP消息头，如果没有设置，则构造新的HTTP消息头。
        if(request == null){
